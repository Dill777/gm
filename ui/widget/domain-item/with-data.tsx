...
export const DomainItemWithData = (
  {
    rowIndex,
    domainName = "",
    price,
    ...
    chainId,
    symbol,
  }: Domain & { rowIndex: number; price: string; id: string; symbol: string } // price
) => {
  ...
  return (
    <div
      className={cn(
        ...
        "px-5 py-3 tablet:px-3 tablet:pt-7 tablet:pb-5 tablet:flex-col tablet:justify-between tablet:h-[136px] tablet:relative"
      )}
    >
      <div className="flex items-center h-full gap-2 tablet:self-stretch tablet:h-fit">
        ...
        <div className="flex items-center gap-4 tablet:w-full tablet:justify-between">
          <DomainAvatar
            chainId={chainId}
            domainId={domainId}
            className={cn("w-[36px] h-[36px] tablet:hidden")}
          />
